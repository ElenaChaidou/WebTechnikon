...
public class RepairRepository implements RepairRepositoryInterface<Repair, Long, Date> {
    ...
    public List<Repair> findByOwnerId(Long ownerId) {
        try {
            ...
            // Query to find repairs for properties belonging to a specific owner
            TypedQuery<Repair> query = entityManager.createQuery(
                    "SELECT repair FROM Repair repair " +
                    "JOIN repair.property property " +
                    "WHERE property.owner.ownerId = :ownerId", Repair.class);
            ...
            log.error("Error finding repairs for owner ID: " + ownerId, e);
        }
